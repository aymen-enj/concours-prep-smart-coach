...
export const getUserStatistics = async (userId: string): Promise<UserStatistics> => {
  ...
  const successfulAttempts = completedAttempts.filter(a => a.score >= 60).length;
  ...
  const subjectPerformance = Array.from(subjectMap.entries()).map(([subject, info]) => ({
    subject,
    score: Math.round(info.scores.reduce((a, b) => a + b, 0) / info.scores.length),
    change: Math.round(Math.random() * 10 - 5), // Simulation du changement pour maintenant
